...
const redirectUri =
  ...
  "https://pp-cup-final-pp-b5fb.twc1.net/auth/callback";
  ...
  for (let participant of data) {
    try {
      const res = await axios.get(
        `https://osu.ppy.sh/api/v2/users/${participant.Nickname}/osu`,
        {
...
app.get("/auth/login", (req, res) => {
  const authUrl = `https://osu.ppy.sh/oauth/authorize?client_id=${osuClientId}&redirect_uri=${encodeURIComponent(
    redirectUri
